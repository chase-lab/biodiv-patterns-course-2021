  ...
  if(meta.type == "NM"){
    for(i in 1:G){
      ...
      if(nrow(invdf)>0){
        ...
        winsp <- invdf %>% group_by(dest) %>% summarize(sp=inv.compet(sp.ini, sp=sp, dest=dest, type="NM"), .groups = 'drop')
    ...
    for(i in 1:G){
      ...
      if(nrow(invdf)>0){
        ...
        winsp <- invdf %>% group_by(dest) %>% summarize(sp=inv.compet(
          sp.ini, yy.data=yy.data, opt=opt, sd=sd, sp=sp, dest=dest, type="SS"), .groups = 'drop')
        sp.ini[winsp$dest] <- winsp$sp
      }
    }
  }
